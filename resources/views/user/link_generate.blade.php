...
@section('content')
    ...
    <div class="for-content-tabs">
        <h2 class="top-tabt-heading">Links</h2>
        <br>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        @foreach ($user_links as $user_link)
                            @foreach ($user_link->get_user_links as $link)
                                <tr>
                                    <th scope="row">{{ $link->id ?? '' }}</th>
                                    <td class="link_copy_td">
                                        <input type="text" id="copy_{{ $link->id }}" class="link_copy_input"
                                            value="{{ route('login', ['link' => $link->generated_link]) ?? '' }}"
                                            readonly>
                                        <button class="badge badge_logo_clr link_copy_btn"
                                            id="copyLinkBtn{{ $link->id }}"
                                            onclick="copyLink({{ $link->id }},'copyLinkBtn{{ $link->id }}')"><i
                                                class="fa fa-clone" aria-hidden="true"></i></button>
                                        <span id="copied_{{ $link->id }}" class="copied_span ">Copied</span>
                                    </td>
                                    <td class="password_copy_td">
                                        @if (!empty($link->actual_password))
                                            <input type="text" id="copyPass_{{ $link->id }}"
                                                class="link_pass_copy_input" value="{{ $link->actual_password ?? 'N/A' }}"
                                                readonly>
                                            <button class="badge badge_logo_clr link_copy_btn"
                                                id="copyPasswordBtn{{ $link->id }}"
                                                onclick="copyPassword({{ $link->id }},'copyPasswordBtn{{ $link->id }}')"><i
                                                    class="fa fa-clone" aria-hidden="true"></i></button>
                                            <span id="copiedPass_{{ $link->id }}"
                                                class="copied_pass_span copied_span_style lcs_none">Copied</span>
                                        @else
                                            N/A
                                        @endif
                                    ...
                                    <td>
                                        ...
                                        @if ($link->link_used_count == 0)
                                            @if (empty($link->request_for_password))
                                                <span class="badge badge_regenerate" data-bs-toggle="modal"
                                                    data-bs-target="#exampleModal2"
                                                    onclick="get_request_password_id({{ $link->id }})">Request
                                                    Password</span>
                                            @else
                                                <span class="badge success_expire">Requested</span>
                                            @endif
                                        @else
                                            <span class="badge success_status">Active Link</span>
                                        @endif


                                        {{-- <span class="badge badge_delete" onclick="delete_link({{ $link->id }})">Delete Link</span> --}}
                                    </td>
                                </tr>
                            @endforeach
                        @endforeach
